...
The **CDIF Data Structure profile** (`cdifDataStructure`) documents the *organization of values inside a dataset* — how variables play roles in a data structure (dimension, measure, identifier, attribute), how the variables are keyed (primary and foreign keys), and which physical data layout is in use (wide, long, dimensional). It builds on the CDIF Data Description profile (which describes the variables themselves) and adds the structural relationships among them. There are two typical applications of the profile 1.)to represent a data structure as a reusable object that can be referenced from multiple dataset descriptions; and 2.) to represent the data structure of a distribution inline in a dataset description.



This guide documents the profile-specific constraints. The **CDIF Data Structure profile** is combined with core, discovery and data description profiles in a separate document specification **doc-discoverydatadescriptionstructure**. Have a look at that repository to see complete dataset examples.
...
* [Properties added to schema:DataDownload in schema:Dataset](#properties-added-to-schemadatadownload-in-schemadataset)

  * [cdi:isStructuredBy](#cdiisstructuredby)
* [Classes added by this profile](#classes-added-by-this-profile)

  * [cdi:DataStructure and subtypes.](#cdidatastructure-and-subtypes)
...
An instance dataset description that includes this profile must declare conformance:
...
and must include at least one `schema:distribution` of type `schema:DataDownload` whose `cdi:isStructuredBy` value is either an inline instance of one of "cdi:DataStructure", "cdi:DimensionalDataStructure", "cdi:LongDataStructure", or "cdi:WideDataStructure", or an object reference to a web-accessible instance of one of these objects. At least one such download is required, not every one: a record may also distribute documentation, a checksum file or a landing page, none of which have a structure to declare.

Three placements satisfy this, and any one of them is enough:

- **On the download** — the ordinary case, where the distribution is the data.
- **On a WebAPI action result** — a `schema:WebAPI` carries no data itself; the thing that does is the `schema:result` of its `schema:potentialAction`, which is typed `schema:DataDownload`.
- **On the parts of a bundle** — a distribution holding several datasets asserts the structure on each `schema:hasPart`, not on the container, since one structure over the whole archive would say something of the file that is true of only some of its members. Where several parts share a layout, state it inline on one and carry an `@id` reference to that node on the others. Note that the parts are defined by the **manifest** profile (`schema:MediaObject`, deliberately not `schema:DataDownload` — they are not independently retrievable). A record whose distribution carries `schema:hasPart` **must** therefore also declare conformance to `https://w3id.org/cdif/manifest/1.1`, and the schema enforces this: without the manifest profile nothing validates the parts themselves, so a structure could be asserted on a part that no rule has checked. The requirement is triggered by the presence of `schema:hasPart` on any distribution, not by where the structure happens to be attached.

### Standalone DataStructure

A standalone DataStructure Object is a JSON-LD document that has a root @type that is one of "cdi:DataStructure","cdi:DimensionalDataStructure", "cdi:LongDataStructure", or "cdi:WideDataStructure".  This document can be referenced for use in other metadata records using its @id identifier, which must dereference on the web to GET the DataStructure object.
...
* **Cardinality:** Required, for at least one conforming DataDownload in a Dataset document instance. The DataDownload might be a direct child of `schema:distribution`, the `schema:result` of a WebAPI action, or a `schema:hasPart` member of a bundle distribution. Repeatable: a single file holding more than one dataset -- a NeXus file with both a dimensional view and the raw wide table -- carries an array of structures, and neither is "the" structure of the file.
* **Content:** one of: an inline `cdi:DataStructure`, `cdi:DimensionalDataStructure`, `cdi:LongDataStructure`, or `cdi:WideDataStructure`; or an `@id` reference to a DataStructure object defined elsewhere (but accessible on the web); or an array of either.
...
* **Cardinality:** Optional
* **Content:** string
* **Description:** Identifier scheme (e.g., DOI, ARK). Values from [https://registry.identifiers.org/registry/](https://registry.identifiers.org/registry/) are recommended for interoperability.
